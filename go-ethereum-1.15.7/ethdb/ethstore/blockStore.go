...
func (aol *AppendOnlyLog) rebuildSkiplist() error {
    ...
    common.SortUint64sReverse(allBlockIDs) // Using common.SortUint64sReverse
...
func setupAppendLog(t *testing.T, dir string, recentN int) *AppendOnlyLog {
    ...
    log.Root().SetHandler(lvlHandler)

    // Pass the root logger (which now uses the filtered handler)
    aol, err := NewAppendOnlyLog(dir, recentN, log.Root())
    if err != nil {
...
// --- Helper for sorting uint64 reverse (if not using common package) ---
// type sortableUint64sReverse []uint64
// func (s sortableUint64sReverse) Len() int           { return len(s) }
// func (s sortableUint64sReverse) Swap(i, j int)      { s[i], s[j] = s[j], s[i] }
// func (s sortableUint64sReverse) Less(i, j int) bool { return s[i] > s[j] } // Reverse sort
