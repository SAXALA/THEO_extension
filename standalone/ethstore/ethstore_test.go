...
func TestEthStore(t *testing.T) {
	t.Run("DatabaseSuite", func(t *testing.T) {
		dbtest.TestDatabaseSuite(t, func() ethdb.KeyValueStore {
			...
			db, err := New(tempDir, 10, "testdb", false, false)
			if err != nil {
...
func BenchmarkEthStore(b *testing.B) {
	dbtest.BenchDatabaseSuite(b, func() ethdb.KeyValueStore {
		...
		db, err := New(tempDir, 10, "benchdb", false, false)
		if err != nil {
...
func TestEthStore_Lifecycle(t *testing.T) {
	t.Run("OpenInMemoryAndWrap", func(t *testing.T) {
		...
		store, err := New(tempDir, 10, "lifecycle_mem_test", false, false)
	...
	t.Run("OpenPersistentWrapAndReopen", func(t *testing.T) {
		...
		store1, err := New(dbPath, 10, "lifecycle_persist_test", false, false)
		...
		store2, err := New(dbPath, 10, "lifecycle_persist_test", false, false) // Reopen
...
func TestEthStore_SpecificBlockOperations(t *testing.T) {
	tempDir := t.TempDir()
	store, err := New(tempDir, 10, "specific_ops_test", false, false) // Using temp dir
...
func TestOther(t *testing.T) {
	tempDir := t.TempDir()
	store, err := New(tempDir, 10, "put_test", false, false)
	require.NoError(t, err)
	require.NotNil(t, store)
	defer store.Close()
