...
const (
	...
	pendingMagic     = "PKV3"
	pendingRecKV     = byte(0x01) //a single kv record
	pendingRecFIN    = byte(0x02) //finish marker
)

// TxIndexAppendOnlyLog implements the append-only log store with skiplist indexing for recent blocks.
type TxIndexAppendOnlyLog struct {
	dirPath string
	log     log.Logger
...
func NewAppendOnlyLog(dirPath string, recentN int, logger log.Logger) (*TxIndexAppendOnlyLog, error) {
	if recentN <= 0 {
	...
	aol.log.Info("TxIndexAppendOnlyLog initialized", "dataSize", common.StorageSize(currentOffset), "indexedBlocks", len(aol.indexedBlocks), "recentBlocksTracked", len(aol.recentBlocks))
...
func (aol *TxIndexAppendOnlyLog) Path() string {
	return aol.dirPath
}

// RecentN returns the number of recent blocks indexed in the skiplist.
func (aol *TxIndexAppendOnlyLog) RecentN() int {
	return aol.recentN
}

// loadBlockIndex reads the index map file into memory.
func (aol *TxIndexAppendOnlyLog) loadBlockIndex() error {
...
func (aol *TxIndexAppendOnlyLog) rebuildSkiplist() error {
...
func (aol *TxIndexAppendOnlyLog) readAndIndexBlock(indexEntry blockIndexEntry) error {
...
func (aol *TxIndexAppendOnlyLog) Append(blockID uint64, kvs map[string]string) error {
	aol.mu.Lock()
	defer aol.mu.Unlock()
...
func (aol *TxIndexAppendOnlyLog) evictOldEntries() {
...
func (aol *TxIndexAppendOnlyLog) Get(key string) (string, bool, error) {
	aol.mu.RLock()
...
func (aol *TxIndexAppendOnlyLog) Delete(key string) error {
	aol.mu.Lock()
	defer aol.mu.Unlock()
...
func (aol *TxIndexAppendOnlyLog) DeleteByPrefixInBlock(targetBlockID uint64, prefix string) error {
	var blockData []byte
	var readErr error
...
func (aol *TxIndexAppendOnlyLog) GetByBlock(blockID uint64) (map[string]string, error) {
	aol.mu.RLock()
	defer aol.mu.RUnlock()
...
func (aol *TxIndexAppendOnlyLog) writeLogEntry(w io.Writer, blockID uint64, key, value string) error {
	keyBytes := []byte(key)
...
func (aol *TxIndexAppendOnlyLog) readLogEntry(r io.Reader) (*logEntry, int64, error) {
...
func (aol *TxIndexAppendOnlyLog) writeIndexEntry(w io.Writer, entry blockIndexEntry) error {
...
func (aol *TxIndexAppendOnlyLog) persistIndexMap() error {
...
func (aol *TxIndexAppendOnlyLog) AppendToNewBlock(kvs map[string]string) (uint64, error) {
	aol.mu.Lock()
	defer aol.mu.Unlock()
...
func (aol *TxIndexAppendOnlyLog) readValueBytesFromPointer(pointer *kvPointer) ([]byte, error) {
...
func (aol *TxIndexAppendOnlyLog) Close() error {
	aol.mu.Lock()
	defer aol.mu.Unlock()
	...
	aol.closed = true
	fmt.Println("Closing TxIndexAppendOnlyLog...")
...
func (aol *TxIndexAppendOnlyLog) bufferIndexEntry(entry blockIndexEntry) error {
...
func (aol *TxIndexAppendOnlyLog) flushIndexBuffer() error {
	aol.indexBufferMu.Lock()
...
func (aol *TxIndexAppendOnlyLog) getBlockIndexEntry(blockID uint64) (blockIndexEntry, bool) {
