...
func (wb *WriteBatch) EnableAutoCommit(db *SSPrefixDB, threshold int) {
	wb.lock.Lock()
...
func (wb *WriteBatch) EnableBackgroundCommit(db *SSPrefixDB) {
	wb.lock.Lock()
	defer wb.lock.Unlock()
...
func (wb *WriteBatch) checkAndCommit() {
	...
	if needCommit {
		if wb.bgCommit {
			...
			default:
				if wb.db != nil {
					_ = wb.db.WriteCommit(batchToCommit)
				}
			}
		} else {
			if wb.db != nil {
...
func (wb *WriteBatch) addSlot(slotIndex int64, slotData map[string][]byte) {
	if slotIndex < 0 {
		return
	}
...
func (db *SSPrefixDB) WriteCommit(batch *WriteBatch) error {
