...
func (nc *NodeCache) CachePathToNode(key string, db *SSPrefixDB) {
	nc.lock.Lock()
	defer nc.lock.Unlock()
	...
	for len(currentKey) > 0 {
		...
		if element, exists := nc.cache[currentKey]; exists {
			...
			nodeValue, slotIndices, err := db.readFromFile(offset, TASnapshot)
			if err != nil {
...
type SlotCache struct {
	...
	lock     sync.RWMutex
	db       *SSPrefixDB // Reference to PrefixDB for batch operations
}
...
func NewSlotCache(capacity int, db *SSPrefixDB) *SlotCache {
	return &SlotCache{
		capacity: capacity,
...
func (nc *NodeCache) batchCachePathNodes(nodes map[string]struct{}, db *SSPrefixDB) {
	nc.lock.Lock()
	defer nc.lock.Unlock()
...
func (nc *NodeCache) AsyncCachePathToNode(key string, db *SSPrefixDB) {
	if !nc.isWorkerRunning {
		nc.startWorker()
	}
...
func (nc *NodeCache) SyncCachePathToNode(key string, db *SSPrefixDB) {
	if !nc.isWorkerRunning {
		nc.startWorker()
	}
